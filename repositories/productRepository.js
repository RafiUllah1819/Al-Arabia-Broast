...
export async function createProduct({
  ...
  image_url, is_available, is_active, sort_order,
}) {
  const result = await query(
    ...
    [category_id || null, name, slug, barcode || null, description || null, type,
     base_price || null, image_url, is_available, is_active, sort_order ?? 0]
  );
...
export async function updateProduct(id, {
  ...
  const result = await query(
    ...
    [category_id || null, name, slug, barcode || null, description || null, type,
     base_price || null, image_url, is_available, is_active, sort_order ?? 0, id]
  );
  return result.rows[0];
}
